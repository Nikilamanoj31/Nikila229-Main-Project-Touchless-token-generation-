    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            if cleaned_landmark:
                ...
                if ticket_counter:
                    if ticket_counter[-1] == y_pred:
                        ticket_counter.append(y_pred)
                    else:
                        ticket_counter = [y_pred]
                else:
                    ticket_counter.append(y_pred)
                if len(ticket_counter)>40:

                    print("Ticket Generated")
                    global result
                    ticket_numer+=1
                    result = "Ticket Generated" + str(ticket_numer)
                    ticket_counter = [y_pred]
                    image = cv2.putText(image, result, (150,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
